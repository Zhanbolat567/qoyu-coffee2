  ...
  return (
    <div className="mx-auto max-w-screen-2xl">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div />
        <button onClick={clearHistory} className="px-3 py-1.5 rounded-md bg-slate-200 hover:bg-slate-300 text-sm" title="Очистить список (и скрыть на табло «ГОТОВ»)">
          Очистить историю
        </button>
      </div>
          ...
          <div key={o.id} className="bg-white rounded-lg shadow overflow-hidden">
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 grid place-items-center rounded-md bg-slate-100 font-semibold">{o.id}</div>
                <div className="font-semibold">{o.customer_name || "Гость"}</div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-slate-500 text-sm">{mmssSince(o.created_at)}</div>
                <span className="inline-block text-xs px-2 py-0.5 rounded-md bg-slate-100 text-slate-700">закрыт</span>
              </div>
            </div>
                    ...
                    {orderedChips.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-x-2 gap-y-1">
                        {orderedChips.map((label) => (
                          <span key={label} className="inline-flex items-center px-2 py-1 text-[12px] leading-5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 whitespace-nowrap">
                            {label}
                          </span>
                        ))}
                      </div>
