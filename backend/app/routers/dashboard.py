...
def stats(db: Session = Depends(get_db)):
    now = func.now()
    day_sum = db.query(func.coalesce(func.sum(models.Order.total), 0))\
        .filter(func.date(models.Order.created_at) == func.date(now)).scalar() or 0
    month_sum = db.query(func.coalesce(func.sum(models.Order.total), 0))\
        .filter(
            extract('year', models.Order.created_at) == extract('year', now),
            extract('month', models.Order.created_at) == extract('month', now)
        ).scalar() or 0
    day_count = db.query(func.count(models.Order.id))\
        .filter(func.date(models.Order.created_at) == func.date(now)).scalar() or 0
    month_count = db.query(func.count(models.Order.id))\
        .filter(
            extract('year', models.Order.created_at) == extract('year', now),
            extract('month', models.Order.created_at) == extract('month', now)
        ).scalar() or 0
...
def hourly(db: Session = Depends(get_db)):
    rows = db.query(
        extract('hour', models.Order.created_at).label('h'),
...
def recent(limit: int = 5, db: Session = Depends(get_db)):
    rows = db.query(models.Order)\
             .order_by(models.Order.created_at.desc())\
             .limit(limit).all()
    # ВАЖНО: отдаем guest_seq как id
    return [
        RecentOrder(
            id=o.guest_seq,
